...
export async function changeUserPassword(data: Omit<PasswordChangeInput, "confirm_password">) {
  ...
  try {
    ...
    return {
      success: false,
      error: "Password changes should be done through your account settings. Please use the account management menu.",
    };
  } catch (error: any) {
    console.error("Error changing password:", error);
    return {
      success: false,
      error: error.errors?.[0]?.longMessage || "Failed to change password",
    };
  }
}
