...
export default async function EditArticlePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const article = articles.find((a) => a.id === Number.parseInt(resolvedParams.id))

  // Find author ID from author name
  const authorId = writers.find((w) => w.name === article?.author)?.id.toString() || ""

  const [formData, setFormData] = useState({
    title: article?.title || "",
    slug: article?.slug || "",
    category: article?.category || "COUNTRY",
  ...
  const handleSubmit = async (e: React.FormEvent, status: "draft" | "published") => {
    ...
    console.log("[v0] Updating article:", resolvedParams.id, { ...formData, content, status })
