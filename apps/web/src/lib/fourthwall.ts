...
const FOURTHWALL_API_URL = process.env.NEXT_PUBLIC_FW_API_URL || "https://storefront-api.fourthwall.com/v1"
...
function transformProduct(fwProduct: FourthwallProduct): Product {
  ...
  return {
    ...
    availableForSale: fwProduct.variants.some(
      (v) => v.stock.type === "Unlimited" || (v.stock.type === "Limited" && (v.stock.inStock || 0) > 0),
    ),
    featuredImage: {
    ...
    variants: fwProduct.variants.map((v) => ({
      id: v.id,
      title: v.name,
      availableForSale: v.stock.type === "Unlimited" || (v.stock.type === "Limited" && (v.stock.inStock || 0) > 0),
      selectedOptions: [
...
function transformCart(fwCart: FourthwallCart, currency: string): Cart {
  const lines = fwCart.items.map((item) => {
    ...
    return {
      ...
      merchandise: {
        ...
        selectedOptions: [
          ...(item.variant.attributes.color ? [{ name: "Color", value: item.variant.attributes.color.name }] : []),
          ...(item.variant.attributes.size ? [{ name: "Size", value: item.variant.attributes.size.name }] : []),
        ],
        product: {
  ...
  const totalAmount = lines.reduce((sum, line) => sum + Number.parseFloat(line.cost.totalAmount.amount), 0)
...
export async function getCart(cartId: string | undefined, currency = "USD"): Promise<Cart | undefined> {
  if (!cartId) {
    return undefined
  }
