...
import { Check, Edit, Eye, EyeOff, Loader2, Plus, Trash2, Upload, X } from "lucide-react";
...
function AdminArticles() {
  ...
  const coverUploadMutation = useMutation({
    ...
    onSuccess: (result) => {
      ...
      setCoverImage(getPublicUploadUrl(key));
    },
  });
                  ...
                  <div className="flex flex-wrap gap-2">
                    <label className="inline-flex h-10 cursor-pointer items-center gap-2 rounded-lg border border-gray-800 px-3 font-black text-white text-xs uppercase tracking-[0.14em] hover:border-[#7CFC00] hover:text-[#7CFC00]">
                      ...
                      <input
                        ...
                        onChange={(event) => {
                          ...
                          if (files.length > 0) {
                            coverUploadMutation.mutate(files);
                          }
                          event.target.value = "";
                        }}
                      />
                    ...
                    {coverImage ? (
                      <button
                        ...
                        className="inline-flex h-10 items-center gap-2 rounded-lg border border-gray-800 px-3 text-gray-300 text-xs uppercase tracking-[0.14em] hover:border-red-500 hover:text-red-300"
                      >
                        <X className="size-4" />
                        Remove
                      </button>
